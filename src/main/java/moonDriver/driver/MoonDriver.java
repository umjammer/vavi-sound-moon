...
public class MoonDriver    {
    ...
    private static final Logger logger = getLogger(MoonDriver.class.getName());
        ...
        public GD3 getGD3Info(MmlDatum[] buf, int vgmGd3)
        {
            ...
            int adrTag;
            adrTag = (short)(buf[0x2e].dat + buf[0x2f].dat * 0x100);
            if (adrTag != 0)
            {
                adrTag -= 0x8000;
        ...
        public boolean init(MmlDatum[] vgmBuf, Consumer<ChipDatum> WriteOPL4Register ,double SampleRate)
        {
            ...
            logger.log(Level.INFO, String.format("MoonDriverDotNET  VER yymmdd Programed by Kuma"));
        ...
        private final byte RAM_PAGE3 = 0xFE;
        ...
        private final short MDR_ID = 0x8000;
        private final short MDR_PACKED = 0x802A;// 1 if packed

        private final short MDR_DSTPCM = 0x8030;// destination address of PCM
        private final short MDR_STPCM = 0x8031;// PCM start bank
        private final short MDR_PCMBANKS = 0x8032;// PCM banks
        private final short MDR_LASTS = 0x8033;// PCM size of lastbank


        private final short S_DEVICE_FLAGS = 0x8007;

        private final short S_TRACK_TABLE = 0x8010;
        ...
        private short[] fm_fnumtbl = new short[] {
            345 // C 523.300000
	        ,365 // C+ 554.400000
	        ,387 // D 587.300000
	        ,410 // D+ 622.300000
        ...
        //********************************************
        // BSMCH
        private short[] fm_drum_fnum = new short[] {
    0x0120 // B
    ,0x0150 // S
    ,0x01c0 // M
    ,0x01c0 // C
    ...
    ,0x02 // H
        ...
        private short[] freq_table = new short[]
        {
...
//#endif
        ...
        private void get_table_hl_2de()
        {
            ...
            hl++;
            hl = (short)((byte)ReadMemory(hl).dat * 0x100 + a);
        ...
        private void seq_rest()
        {
            ...
            if (work.ch[ix].cnt == 255)
            {
                ...
                byte v00 = (byte)ReadMemory((short)(hl + 1)).dat;
                short adr = (short)(ReadMemory((short)(hl + 2)).dat + ReadMemory((short)(hl + 3)).dat * 0x100);
        ...
        private void seq_bank()
        {
            ...
            a = 0;
            change_page3();
            short ltbl = (short)((byte)ReadMemory(S_LOOP_TABLE).dat + (byte)ReadMemory(S_LOOP_TABLE + 1).dat * 0x100 + ix * 2);
        ...
        {
            ChipDatum cd = new ChipDatum(0 * 0x100 + 0, d, e);
            WriteOPL4Register(cd);
        }

        private void moon_fm2_out()
        {
            ChipDatum cd = new ChipDatum(0 * 0x100 + 1, d, e);
            WriteOPL4Register(cd);
        }

        private void moon_wave_out()
        {
            ChipDatum cd = new ChipDatum(0 * 0x100 + 2, d, e);
            WriteOPL4Register(cd);
            backDat = e;
        }
